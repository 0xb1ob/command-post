...
Supersession (2026-08-21, [#13](https://github.com/0xb1ob/command-post/issues/13)): the runtime worker/worktree/branch map now lives in `bin/cmdp jobs`. The `kind=`/`delivery=` passthrough described below was muxa's CLI requirement and is gone; those fields stay on the br issue.
...
3. At spawn, record worker + worktree in `bin/cmdp jobs` only (runtime ledger;
   keyed by br id). Put the PR URL on `br close`, not on `bin/cmdp jobs done`.
...
bin/cmdp jobs add cp-cluster-reads-ab12 worker=lively-comet worktree=<path>
# completion
br close cp-cluster-reads-ab12 --reason "PR: https://github.com/…/pull/22" --json
bin/cmdp jobs done cp-cluster-reads-ab12
