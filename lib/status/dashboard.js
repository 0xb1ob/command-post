...
(function (global) {
  ...
  var SNAPSHOT_PANE_MSG =
    ...
    "Run bin/cmdp status --serve for live pane preview.";
  ...
  var previewScrollByNode = {};
  var TIME_SOURCE_LABEL = {
    dispatched_at: "jobs.tsv dispatched_at (exact job start, stamped at bin/cmdp dispatch)",
  ...
  function closePreview() {
    capturePreviewScroll(preview.nodeId);
    preview.open = false;
    preview.nodeId = null;
    preview.full = false;
    preview.lines = [];
  ...
  function previewTermEl() {
    var modal = document.getElementById("fleet-preview-modal");
    return modal ? modal.querySelector(".fleet-preview-term") : null;
  }

  function termHasRealContent(term) {
    var lines = term.querySelectorAll(".fleet-preview-line");
    if (!lines.length) return false;
    if (lines.length === 1 && lines[0].classList.contains("dim")) return false;
    if (lines.length === 1 && lines[0].classList.contains("warn")) return false;
    return true;
  }

  function capturePreviewScroll(nodeId) {
    if (!nodeId) return;
    var term = previewTermEl();
    if (!term || !termHasRealContent(term)) return;
    var maxScroll = term.scrollHeight - term.clientHeight;
    var atBottom = maxScroll <= SCROLL_BOTTOM_THRESHOLD ||
      maxScroll - term.scrollTop <= SCROLL_BOTTOM_THRESHOLD;
    var next = { scrollTop: term.scrollTop, atBottom: atBottom };
    var prev = previewScrollByNode[nodeId];
    if (prev && !prev.atBottom && prev.scrollTop > SCROLL_BOTTOM_THRESHOLD &&
        next.atBottom && next.scrollTop <= SCROLL_BOTTOM_THRESHOLD) {
      return;
    }
    previewScrollByNode[nodeId] = next;
  }

  function restorePreviewScroll(nodeId) {
    if (!nodeId) return;
    var saved = previewScrollByNode[nodeId];
    if (!saved) return;
    var term = previewTermEl();
    if (!term) return;
    if (saved.atBottom) {
      ...
      term.scrollTop = saved.scrollTop;
    }
  }

  function schedulePreviewScrollRestore(nodeId) {
    function run() { restorePreviewScroll(nodeId); }
  ...
  function shouldShowStreamingCaret(nodeId, streaming) {
    if (!streaming) return false;
    var term = previewTermEl();
    if (term && termHasRealContent(term)) {
      var maxScroll = term.scrollHeight - term.clientHeight;
      if (maxScroll - term.scrollTop > SCROLL_BOTTOM_THRESHOLD) return false;
    }
    var saved = previewScrollByNode[nodeId];
    if (saved && !saved.atBottom) return false;
    return true;
  }
  ...
  function renderPreviewModal(data, opts) {
    ...
    if (shouldShowStreamingCaret(preview.nodeId, streaming)) {
    ...
    schedulePreviewScrollRestore(preview.nodeId);
  ...
  function statusIcon(phase, brokerOk) {
    ...
    if (phase === "blocked") return "<span class=\"fleet-status-stalled\" title=\"blocked\">\u26d4</span>";
